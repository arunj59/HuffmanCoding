   ...
   */
    def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      def charEncode(tr: CodeTree)(ch: Char):List[Bit] = tr match {
        case Leaf(c,w) => List()
        case Fork(l,r,c,w) => if (chars(l).contains(ch)) 0 :: charEncode(l)(ch) else 1 :: charEncode(l)(ch)
      }

      def _encode(text: List[Char]):List[List[Bit]] = text match {
        case Nil => Nil
        case x :: xs => charEncode(tree)(x) :: _encode(xs)
      }

      _encode(text).flatten
    }
